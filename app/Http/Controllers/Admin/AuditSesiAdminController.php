...
{
    /**
     * Display a listing of audit sessions owned by the admin user.
     */
    public function index(Request $request)
    {
        $query = AuditSesi::where('user_id', auth()->id())->latest();
    ...
    /**
     * Store a newly created audit session in storage (admin context).
     */
    public function store(Request $request)
    {
        $request->validate([
    ...
    /**
     * Display matrix scoring view for admin.
     * Shares the auditor matrix view since it's role-agnostic.
     */
    public function matrix($id)
    {
        $sesi = AuditSesi::where('user_id', auth()->id())->findOrFail($id);
    ...
    /**
     * Update matrix scores (admin context, with admin route redirects).
     */
    public function updateMatrix(Request $request, $id)
    {
        $sesi = AuditSesi::where('user_id', auth()->id())->findOrFail($id);
    ...
    /**
     * Display printable report (shared auditor view, access-checked).
     */
    public function cetak($id)
    {
        $sesi = AuditSesi::with(['user', 'auditDetails.kriteria.subElemen.elemen'])->findOrFail($id);
    ...
    /**
     * Export to Excel (access-checked).
     */
    public function exportExcel($id)
    {
        $sesi = AuditSesi::with(['user', 'auditDetails.kriteria.subElemen.elemen'])->findOrFail($id);
    ...
    /**
     * Finalize audit session (admin context).
     */
    public function finalisasi($id)
    {
        $sesi = AuditSesi::where('user_id', auth()->id())->findOrFail($id);
