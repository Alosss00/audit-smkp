                    ...
                    @foreach($el['sub_elemens'] as $sub)
                        @if(!empty($sub['is_direct']))
                            ...
                            <tr class="collapse show el-collapse-{{ $el['elemen_id'] }} bg-sub-elemen">
                                ...
                                <td class="text-center text-muted">-</td>
                                <!-- Cetak Nilai Maksimal di kolom Nilai Sub Elemen (kolom ke-5) -->
                                <td class="text-center font-monospace fw-bold">{{ number_format($sub['total_nilai_maks_efektif'], 0) }}</td>
                                <td class="text-center text-muted">-</td>
                                <!-- Cetak Nilai Hasil Audit di kolom Nilai Sub Elemen (kolom ke-7, biru) -->
                        ...
                        @else
                            ...
                            <tr class="collapse show el-collapse-{{ $el['elemen_id'] }} bg-sub-elemen" style="cursor: pointer;" data-bs-toggle="collapse" data-bs-target=".sub-collapse-{{ $sub['sub_elemen_id'] }}">
                                ...
                                <td class="text-center text-muted">-</td>
                                <!-- Nilai Maksimal Total Sub-Elemen -->
                                <td class="text-center font-monospace fw-bold">{{ number_format($sub['total_nilai_maks_efektif'], 0) }}</td>
                                <td class="text-center text-muted">-</td>
                                <!-- Nilai Aktual Total Sub-Elemen (biru) -->
...
<!-- Section Temuan Inkonsistensi Cross-Check / Gating Logic -->
@if(isset($gatingViolations) && $gatingViolations->isNotEmpty())
    <div class="card card-custom p-4 mb-4 border-start border-5 border-warning shadow-sm">
        <div class="d-flex align-items-center justify-content-between mb-3 pb-2 border-bottom flex-wrap gap-2">
            <div class="d-flex align-items-center gap-3">
                <div class="stat-icon-box bg-warning bg-opacity-10 text-warning d-flex align-items-center justify-content-center" style="width: 44px; height: 44px; border-radius: 12px;">
                    <i class="bi bi-shield-exclamation fs-4"></i>
                </div>
                <div>
                    <h5 class="fw-bold text-slate-800 mb-0">Temuan Inkonsistensi Cross-Check / Gating Logic</h5>
                    <p class="text-muted small mb-0">Deteksi inkonsistensi ketergantungan nilai kriteria hulu terhadap kriteria hilir (Metodologi SMKP)</p>
                </div>
            </div>
            <span class="badge bg-warning text-dark px-3 py-2 rounded-pill fw-bold">
                {{ $gatingViolations->count() }} Temuan Inkonsistensi
            </span>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        <th style="width: 130px;">Kriteria Hulu</th>
                        <th style="width: 130px;">Kriteria Hilir</th>
                        <th style="width: 140px;">Batas Patokan</th>
                        <th>Analisis Inkonsistensi</th>
                        <th style="width: 110px;" class="text-center">Mode</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($gatingViolations as $v)
                        <tr>
                            <td>
                                <span class="badge bg-dark font-monospace py-1 px-2">{{ $v['kode_hulu'] }}</span>
                                <div class="small fw-semibold text-slate-700 mt-1">Skor: <span class="badge bg-light text-dark border">{{ $v['skor_hulu'] }}</span></div>
                            </td>
                            <td>
                                <span class="badge bg-primary font-monospace py-1 px-2">{{ $v['kode_hilir'] }}</span>
                                <div class="small fw-semibold text-danger mt-1">Skor: <span class="badge bg-danger text-white">{{ $v['skor_hilir'] }}</span></div>
                            </td>
                            <td>
                                <span class="badge bg-info-subtle text-info-emphasis border border-info-subtle px-2 py-1 rounded-pill small">
                                    Maksimal: {{ $v['skor_maks_hilir'] }}
                                </span>
                            </td>
                            <td>
                                <div class="small fw-semibold text-slate-800">{{ $v['pesan'] }}</div>
                                <small class="text-muted d-block mt-0.5"><i class="bi bi-info-circle me-1"></i>{{ $v['deskripsi_simpul'] }}</small>
                            </td>
                            <td class="text-center">
                                @if($v['mode'] === 'hard_block')
                                    <span class="badge bg-danger badge-role">Hard Block</span>
                                @else
                                    <span class="badge bg-warning text-dark badge-role">Soft Flag</span>
                                @endif
                            </td>
                        </tr>
                    @endforeach
